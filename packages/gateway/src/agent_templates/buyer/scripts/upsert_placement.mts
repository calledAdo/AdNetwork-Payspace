...
//
// Usage: npx tsx upsert_placement.mts \
...
import { BuyerPlacementSchema, BuyerPlacementsFileSchema } from "./memory_schema.mjs";
...
function readPlacements(): JsonRecord[] {
  try {
...
function writePlacementsAtomic(placements: JsonRecord[]): void {
...
function deepMergePlacement(current: JsonRecord, patch: JsonRecord): JsonRecord {
  ...
  if (
    ...
    merged["task_ids"] = { ...(currentTaskIds as JsonRecord), ...(patchTaskIds as JsonRecord) };
  }
  return merged;
}
...
let updatedPlacement: JsonRecord;
if (index === -1) {
  if (!createIfMissing) {
    process.stderr.write(JSON.stringify({
  ...
  updatedPlacement = validated.data as JsonRecord;
