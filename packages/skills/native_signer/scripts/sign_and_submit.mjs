...
//
// Usage:  node sign_and_submit.mjs \
...
if (!AGENT_DIR) {
  process.stderr.write(
    JSON.stringify({ error: "AGENT_DIR environment variable is not set" }) +
      "\n",
  );
  process.exit(1);
}
const MCP_URL = skillsConfig.payspaceMcpUrl;
...
if (!values["build-url"] || !values["body"]) {
  process.stderr.write(
    JSON.stringify({ error: "--build-url and --body are required" }) + "\n",
  );
  process.exit(1);
}
...
if (!buildRes.ok || !built.signing_message) {
  process.stderr.write(
    JSON.stringify({
      error: "build endpoint did not return signing_message",
      detail: built,
    }) + "\n",
  );
  process.exit(1);
...
const raw = secp256k1.sign(msgBytes, Buffer.from(privkey, "hex"), {
  format: "recovered",
});
const signature =
  "0x" +
...
if (!submitRes.ok) {
  process.stderr.write(
    JSON.stringify({ error: "submit failed", detail: result }) + "\n",
  );
  process.exit(1);
}
