...
//
// Usage:  npx tsx scripts/check_assets.mts
...
const data = await res.json() as {
  ...
  cells?: Array<{
    ...
    output: { capacity: string; type?: unknown };
    output_data?: string;
  }>;
};
...
const udtCells:   typeof cells = [];
const plainCells: typeof cells = [];
...
if (udtCells.length > 0) {
  ...
  for (const cell of udtCells) {
    ...
    const type = cell.output.type as Record<string, string>;
