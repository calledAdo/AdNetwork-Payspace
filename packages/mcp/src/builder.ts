...
// xUDT (Extensible UDT) on Pudge testnet
export const XUDT_CODE_HASH =
  "0x25c29dc317811a6f6f3985a7a9ebc4838bd388d19d0feeecf0bcd60f6c0975bb";
export const XUDT_DEP_TX_HASH =
...
function toCccTransaction(tx: UnsignedTx): ccc.Transaction {
  return ccc.Transaction.from({
    ...
    outputs: tx.outputs.map((out) => ({
      ...
      type: out.type
        ? { codeHash: out.type.code_hash, hashType: out.type.hash_type, args: out.type.args }
        : null,
    })),
 ...
 */
async function buildTemplate(tx: UnsignedTx, signerLockArgs: string): Promise<BuildResult> {
  ...
  if (!signHashInfo) throw new Error("getSignHashInfo: no matching input found for signer lock");
...
export async function collectUdtCells(
  ...
  if (udtAmount < neededAmount)
    throw new Error(`insufficient UDT: need ${neededAmount}, have ${udtAmount}`);
...
function xudtDep(): TxCellDep {
  return {
    out_point: { tx_hash: XUDT_DEP_TX_HASH, index: "0x0" },
    dep_type: "code",
  };
}
...
function settlementDep(): TxCellDep {
  return {
    out_point: { tx_hash: requireEnv("SETTLEMENT_LOCK_DEP_TX_HASH"), index: "0x1" },
    dep_type: "code",
  };
}
...
function bookingSpaceTypeDep(): TxCellDep {
  return {
    out_point: { tx_hash: requireEnv("BOOKING_SPACE_TYPE_DEP_TX_HASH"), index: "0x0" },
    dep_type: "code",
  };
}
...
export const AD_POSITION = { banner: 0, sidebar: 1, native: 2, interstitial: 3 } as const;
...
export type PublicationMode = (typeof PUBLICATION_MODE)[keyof typeof PUBLICATION_MODE];
...
export function encodeBookingSpaceData(params: {
  ...
  const gwBytes = params.gatewayUrl ? Buffer.from(params.gatewayUrl, "utf8") : undefined;
  ...
  if (hasIpfs) {
    ...
    if (params.detailsHash) hexToBuffer(params.detailsHash).copy(buf, 126 + gwLen);
  }
  ...
  if (data.length > 92) {
    ...
    if (data.length > ipfsOffset) {
      ...
      if (base.ipfsPresent && data.length >= ipfsOffset + 65) {
        base.detailsCid = bufferToHex(data.subarray(ipfsOffset + 1, ipfsOffset + 33));
        base.detailsHash = bufferToHex(data.subarray(ipfsOffset + 33, ipfsOffset + 65));
      }
    }
  }
...
export async function buildUdtTransfer(
  ...
  const { inputs: udtInputs, udtAmount, ckbCapacity: udtCkb } =
  ...
  const outputs: TxCellOutput[] = [
    { capacity: formatCap(UDT_CELL_CAPACITY), lock: recipientLock, type: udtTypeScript },
  ];
  ...
  outputs.push({ capacity: formatCap(ckbChange), lock: senderLock, type: null });
  outputsData.push("0x");

  const allInputs = [...udtInputs, ...ckbInputs];
  const tx: UnsignedTx = {
    version: "0x0",
    cell_deps: [secp256k1Dep(), xudtDep()],
    header_deps: [],
    inputs: allInputs,
    outputs,
...
export async function buildOpenChannel(
  ...
  const channelCodeHash = requireEnv("SETTLEMENT_LOCK_CODE_HASH");
  const channelHashType = process.env.SETTLEMENT_LOCK_HASH_TYPE ?? "type";
  ...
  const udtChange = udtAmount - amount;
  if (udtChange > 0n) {
    outputs.push({ capacity: formatCap(UDT_CELL_CAPACITY), lock: buyerLock, type: udtTypeScript });
    outputsData.push(bufferToHex(encodeUdtAmount(udtChange)));
  }

  const ckbChange =
    udtCkb + plainCkb - cellCap - (udtChange > 0n ? UDT_CELL_CAPACITY : 0n) - ESTIMATED_FEE;
  outputs.push({ capacity: formatCap(ckbChange), lock: buyerLock, type: null });
  outputsData.push("0x");

  const tx: UnsignedTx = {
    version: "0x0",
    cell_deps: [secp256k1Dep(), xudtDep(), settlementDep()],
    header_deps: [],
    inputs: allInputs,
    outputs,
    outputs_data: outputsData,
...
export async function buildDisputeTx(
  ...
  const cell = await getLiveCell({ tx_hash: outPoint.tx_hash, index: outPoint.index }, true);
  ...
  if (currentArgs.length !== 136) throw new Error("channel lock args must be 136 bytes");
  ...
  witnesses[channelWitnessIdx] = bufferToHex(makeWitnessArgsLock(disputeWitnessData));
...
export async function buildCoopClose(
  ...
  if (args.length !== 136) throw new Error("channel lock args must be 136 bytes");

  const disputeStart = args.readBigUInt64LE(104);
  if (disputeStart !== 0n) throw new Error("channel is in dispute — use dispute/payout flow");
  ...
  const totalUdt = cellData.readBigUInt64LE(0) + (cellData.readBigUInt64LE(8) << 64n);
  if (sellerUdt + buyerUdt !== totalUdt)
    throw new Error(`amounts don't sum to channel total: ${sellerUdt} + ${buyerUdt} != ${totalUdt}`);
  ...
  const sellerLock: TxScript = { code_hash: "0x" + stdCodeHash, hash_type: "type", args: "0x" + sellerBlake160 };
  const buyerLock: TxScript = { code_hash: "0x" + stdCodeHash, hash_type: "type", args: "0x" + buyerBlake160 };

  const { inputs: feeInputs, capacity: feeCap } = await collectPlainCells(callerLockArgs, ESTIMATED_FEE);
  const channelInput: TxCellInput = { since: "0x0", previous_output: outPoint };
  const allInputs = [...feeInputs, channelInput];
  const channelWitnessIdx = allInputs.length - 1;
  ...
  if (buyerSigBuf.length !== 65) throw new Error("buyer_sig must be 65 bytes");
  if (sellerSigBuf.length !== 65) throw new Error("seller_sig must be 65 bytes");

  const witnesses = placeholderWitnesses(allInputs.length);
  witnesses[channelWitnessIdx] = bufferToHex(makeWitnessArgsLock(Buffer.concat([buyerSigBuf, sellerSigBuf])));

  const callerLock = secp256k1Lock(callerLockArgs);
  const channelCkb = hexToU64(cell.output.capacity);
  const feeChange = channelCkb + feeCap - 2n * UDT_CELL_CAPACITY - ESTIMATED_FEE;
  ...
  const tx: UnsignedTx = {
    ...
    inputs: allInputs,
    outputs: [
      { capacity: formatCap(UDT_CELL_CAPACITY), lock: sellerLock, type: cell.output.type },
      { capacity: formatCap(UDT_CELL_CAPACITY), lock: buyerLock, type: cell.output.type },
      { capacity: formatCap(feeChange), lock: callerLock, type: null },
    ],
    outputs_data: [bufferToHex(encodeUdtAmount(sellerUdt)), bufferToHex(encodeUdtAmount(buyerUdt)), "0x"],
    witnesses,
  };
  return { ...await buildTemplate(tx, callerLockArgs), coop_signing_message: coopSigningMessage };
}
...
export async function buildFinalPayout(
  ...
  if (disputeStartTime === 0n) throw new Error("no dispute initiated — cannot payout");

  const deadlineSecs = disputeStartTime + DISPUTE_WINDOW_SECS;
  const tip = await getTipHeader();
  const tipTimeSecs = tip.timestamp / 1000n;
  if (tipTimeSecs < deadlineSecs)
    throw new Error(`dispute window has not expired yet (deadline ${deadlineSecs}s, tip ${tipTimeSecs}s)`);

  const sellerClaim = args.readBigUInt64LE(112) + (args.readBigUInt64LE(120) << 64n);
  const cellData = hexToBuffer(cell.output_data);
  ...
  const witnesses = placeholderWitnesses(allInputs.length);
  witnesses[channelWitnessIdx] = bufferToHex(makeWitnessArgsLock(Buffer.alloc(0)));
  ...
  if (sellerClaim > 0n) {
    outputs.push({ capacity: formatCap(UDT_CELL_CAPACITY), lock: sellerLock, type: cell.output.type });
    outputsData.push(bufferToHex(encodeUdtAmount(sellerClaim)));
    usedCkb += UDT_CELL_CAPACITY;
  }
  if (buyerClaim > 0n) {
    outputs.push({ capacity: formatCap(UDT_CELL_CAPACITY), lock: buyerLock, type: cell.output.type });
    outputsData.push(bufferToHex(encodeUdtAmount(buyerClaim)));
    usedCkb += UDT_CELL_CAPACITY;
  }

  const ckbRemainder = channelCkb + feeCap - usedCkb - ESTIMATED_FEE;
  outputs.push({ capacity: formatCap(ckbRemainder), lock: callerLock, type: null });
  outputsData.push("0x");
...
export async function buildPublishPlacement(
  ...
  const typeCodeHash = requireEnv("BOOKING_SPACE_TYPE_CODE_HASH");
  const typeHashType = process.env.BOOKING_SPACE_TYPE_HASH_TYPE ?? "type";
  ...
  const { inputs, capacity } = await collectPlainCells(
    sellerLockArgs,
    PLACEMENT_CELL_CAPACITY + ESTIMATED_FEE,
  );

  const sellerLock = secp256k1Lock(sellerLockArgs);
  const changeCapacity = capacity - PLACEMENT_CELL_CAPACITY - ESTIMATED_FEE;
  ...
  const tx: UnsignedTx = {
    ...
    outputs: [
      {
        capacity: formatCap(PLACEMENT_CELL_CAPACITY),
        lock: sellerLock,
...
export async function buildBookPlacement(
  ...
  const channelCell = await getLiveCell({ tx_hash: params.channel_tx_hash, index: "0x0" }, true);
  if (!channelCell) throw new Error("settlement channel cell not found or not live at output 0");
  ...
  if (sellerFromChannel.toLowerCase() !== sellerLockArgs.toLowerCase()) {
    throw new Error("settlement channel seller does not match placement seller");
  }
  ...
  const placementInput: TxCellInput = { since: "0x0", previous_output: outPoint };
  ...
  const tx: UnsignedTx = {
    ...
    inputs: allInputs,
    outputs: [
      { capacity: cell.output.capacity, lock: cell.output.lock, type: cell.output.type },
...
async function injectFeeTank(
  ...
  augmented.outputs[augmented.outputs.length - 1]!.capacity = formatCap(changeCapacity);
