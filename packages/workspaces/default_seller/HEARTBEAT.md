...
3. Check the on-chain placement state and settlement cell state.
   - at booking start, validate the settlement cell is live using `payment_channel/scripts/check_channel.mjs`
   - when a new ticket arrives, recover signer using `payment_channel/scripts/receive_signed_ticket.mjs`
