...
- proceed to `streaming` using on-chain booking state checks (channel id binding)
...
6. If `seller_claim` increased, sign and send a new payment ticket.
   - send `settlement_tx_hash` and `settlement_index` to seller at booking start so seller can run `check_channel` before accepting tickets
